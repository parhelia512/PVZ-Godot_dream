...
func init_state(init_status:E_JacksonStatus):
	...
	if init_status == E_JacksonStatus.Enter:
		## 非攻击状态下，攻击力为0
		attack_component.change_attack_value(0, AttackComponentZombieNorm.E_AttackValueFactor.JacksonEnter)
		## 方向
		zombie_009_jackson.body.scale = Vector2(-1,1)
		node_drop.scale = Vector2(-1,1)
	## 非召唤伴舞初始化为Norm状态
	else:
...
func enter_status(new_value:E_JacksonStatus):
	match new_value:
		...
		E_JacksonStatus.Attack:
			## 修改攻击力为原始值
			attack_component.change_attack_value(1, AttackComponentZombieNorm.E_AttackValueFactor.JacksonEnter)
