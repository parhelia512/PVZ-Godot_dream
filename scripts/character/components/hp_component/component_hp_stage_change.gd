...
## 判断body是否需要变化
## curr_hp: 当前剩余血量
func judge_body_change(curr_hp:int, is_no_drop:=false):
	## 死亡并且无变化
	...
	for i in range(boundary_value_hp.size()):
		if curr_hp_stage < i and curr_hp <= boundary_value_hp[i]:
			...
			for j in range(body_change[i].sprite_appear.size()):
				var sprite_appear:Sprite2D = get_node(body_change[i].sprite_appear[j])
				sprite_appear.visible = true

			for j in range(body_change[i].sprite_disappear.size()):
				var sprite_disappear:Sprite2D = get_node(body_change[i].sprite_disappear[j])
...
## curr_hp: 当前剩余血量
func judge_body_change_armor(curr_hp_arm:int, curr_hp:int, is_no_drop:bool=false, is_armor1:bool=true):
