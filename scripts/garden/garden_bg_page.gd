...
@export var garden_bg_type:GardenManager.GardenBgType
var curr_bg_page := 0
...
func _ready() -> void:
	...
	if garden_bg_type == GardenManager.GardenBgType.Aquarium:
...
## 激活当前页植物生产金币
func activate_curr_page_plant():
	for plant_cell:PlantCellGarden in all_plant_cells:
		plant_cell.activate_curr_plant()
	
## 停止当前页植物生产金币
func deactivate_curr_page_plant():
	for plant_cell:PlantCellGarden in all_plant_cells:
		plant_cell.deactivate_curr_plant()
