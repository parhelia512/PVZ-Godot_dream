...
## 子弹可以穿透的数量(-1表示无限穿透)
@export var max_penetration_numbers := 1
var curr_penetration_numbers := 0
## 子弹击中特效
...
func _on_area_2d_area_entered(area: Area2D) -> void:
	if not is_attack:
		...
		if can_attack_zombie_status & zombie.curr_be_attack_status:
...
func _attack_zombie(zombie:Zombie000Base):
	...
	curr_penetration_numbers += 1
	if max_penetration_numbers != -1 and curr_penetration_numbers >= max_penetration_numbers:
		queue_free()
...
func _on_area_2d_2_area_entered(area: Area2D) -> void:
	## 获取子弹升级植物
	var bullet_up_plant:PlantBase = area.owner
	## 如过是火炬树桩
