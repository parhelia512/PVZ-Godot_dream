...
- 4、新角色写继承新代码时一定要注意写super()，尤其是ready()中


# 卡片
所有的卡片在all_card场景中,自动加载（AllCard）  
创建卡片时从AllCard中复制对应的卡片  
...
## 部分组件功能:
- 血量组件(HpComponent)：管理角色血量，控制受击音效，触发角色死亡，血量ui显示
  - 植物：处理本体血量
  - 僵尸：处理本体和防具血量

- 血量状态变化组件(HpStageChangeComponent):
  - 植物：部分植物（坚果、南瓜头）使用，血量到达临界值时改变body状态
  - 僵尸：血量到达临界值时改变body状态，控制是否有掉落（掉手、掉头、防具），同时有防具血量状态变化管理防具血量到达临界值时改变body状态

- 受击碰撞盒组件(BeAttackedBoxComponent)：控制角色受击碰撞盒
  - 植物：单独受击碰撞盒
  - 僵尸：增加攻击时受击碰撞盒，攻击时将攻击时受击碰撞盒向前移动，部分特殊植物（大嘴花、窝瓜）可以检测到

- 身体组件(Body)：角色身体，控制身体颜色、发亮，body位置为（0,0）因为动画使用R2GA转换，所以子节点位置无法修改，使用位置修正节点（BodyCorrect）修正位置
  - 植物：角色精灵图子节点
  - 僵尸：NodeDrop节点(同级组件)有僵尸掉落的物品（断手、断头、掉落防具）

- 攻击射线检测组件(AttackRayComponent)：检测敌人是否可以攻击到，僵尸会改变状态（正常->跳跃）敌人为僵尸时，会连接状态变化信号
	- 攻击组件(AttackComponent): 攻击射线检测到敌人时，攻击组件\角色本体 进行攻击行为
	
- 移动组件(MoveComponent): 僵尸专用组件，根据动画控制的_ground节点位置\根据速度，控制僵尸移动

- 灰烬组件(CharredComponent): 僵尸被炸弹炸死时，若为灰烬炸弹炸死，显示灰烬动画，不是则直接死亡
