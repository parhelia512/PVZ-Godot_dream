...
- - 本体层面: 角色本体层面,为一个平面. 大部分角色节点所在层面
- - body层面: 与地图斜面对应的层面. body\影子\真实受击碰撞箱\特殊组件(爆炸组件\杨桃攻击组件)  

角色检测敌人时,检测受击碰撞箱, 子弹攻击敌人时,子弹检测真实碰撞箱  
...
- 根据groud节点移动的僵尸,在每次动画结束时发射animation_finished信号设置停止移动,下一个动画若为移动动画且有过度时间,第一次移动动画会停止移动,将前一个动画设置为循环(不发射animation_finished信号),设置动画过度属性`break_loop_at_end = true`
